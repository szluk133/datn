...
AI Chatbot (RAG): Chatbot trả lời câu hỏi tài chính sử dụng Qdrant (Vector DB) và LLM.
...
Sao chép file mẫu và cập nhật thông tin:
...
5. Cài đặt Chatbot Service (Python)
...
Đảm bảo Mongodb, Meilisearch, Qdrant đang chạy.

Chạy Backend (be).

Chạy Frontend (fe).

Chạy Crawler (crawler) lần đầu để nạp dữ liệu vào DB.

Khởi động Chatbot (chatbot) để phục vụ tính năng chat.
