...
from schemas import CrawlParams, ScheduleConfig, UserArticleEnrichRequest
...
from services.scheduler_service import start_scheduler, execute_topic_crawl, reschedule_topic_crawl, process_user_articles_ai
...
async def health_check():
    return {"status": "running", "features": ["Hybrid Search", "Auto Topic Crawl", "Vector Search", "Chatbot RAG", "SSE Realtime", "User Articles AI"]}
...
@app.post("/my-articles/enrich", summary="Sinh AI Data cho bài viết người dùng (My Articles)")
async def enrich_my_articles(
    request: UserArticleEnrichRequest
):
    """
    Trigger sinh AI Summary, Sentiment Score, Sentiment Label cho các bài viết 
    trong collection 'my_articles' dựa trên user_id và update_id.
    
    [LOGIC] API này xử lý ĐỒNG BỘ (Synchronous). 
    Client sẽ phải chờ cho đến khi toàn bộ lô bài được xử lý xong.
    """
    # Gọi trực tiếp hàm xử lý (await) thay vì đẩy vào background_tasks
    # Server sẽ block request này cho đến khi hàm trả về kết quả
    count = await process_user_articles_ai(request.user_id, request.update_id)
    
    return {
        "status": "ok",
        "message": f"Đã xử lý xong toàn bộ {count} bài viết.",
